How to run your own Electrum Azart server
========================================
...
Electrum Azart server on Linux. It is structured as a series of steps you need to
...
The most up-to date version of this document is available at:
    ...
    https://github.com/azartpay/electrum-azart-server/blob/master/HOWTO.md
...
document, we assume that user is called 'azart'). We also assume the
azart user has sudo rights, so we use `$ sudo command` when we need to.
...
you also need to run azartd and keep a copy of the full blockchain,
which is roughly 55 GB (February 2016). Ideally you have a machine with 16 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit azartd 
...
only leaving spare cycles for azartd. Fast single core CPU power helps for the initial 
...
### Step 1. Create a user for running azartd and Electrum Azart server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `azartd` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser azart --disabled-password
    $ sudo apt-get install git
    $ sudo su - azart
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/azart/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download azartd

Recommend downloading latest version directly from Azart.org

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config libevent-dev
    $ sudo su - azart
    $ cd ~/src && wget https://www.azart.org/binaries/azart-0.12.0.57-linux64.tar.gz
    $ sha256sum azart-0.12.0.57-linux64.tar.gz | grep 2a29b529c56d2ba41e28dfb20872861d6b48bdfe4fb327bfd2273123b38139aa
    $ tar xfz azart-0.12.0.57-linux64.tar.gz
    $ cd azart-0.12.0/bin
    $ cp -a azartd azart-cli azart-tx ~/bin

### Step 3. Configure and start azartd

In order to allow Electrum Azart to "talk" to `azartd`, we need to set up an RPC
username and password for `azartd`. We will then start `azartd` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.azart
    $ $EDITOR ~/.azart/azart.conf

Write this in `azart.conf`:
...
If you have an existing installation of azartd and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ azartd -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `azartd`:

    $ azartd

Allow some time to pass for `azartd` to connect to the network and start
downloading blocks. You can check its progress by running:

    $ azart-cli getblockchaininfo

Before starting the Electrum Azart server your azartd should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start azartd at boot
time, running as the 'azart' user. Check your system documentation to
find out the best way to do this.

### Step 4. Download and install Electrum Azart server

We will download the latest git snapshot for Electrum to configure and install it:

    $ cd ~
    $ git clone https://github.com/azartpay/electrum-azart-server.git
    $ cd electrum-azart-server
...
### Optional Step 5: Install Electrum Azart dependencies manually

Electrum Azart server depends on various standard Python libraries and leveldb. These will usually be
...
Electrum Azart server uses leveldb to store transactions. You can choose
...
and further instructions from the Electrum Azart full archival server foundry at a later time.
...
server.key corresponds to `ssl_keyfile=` in your Electrum Azart server config.
...
### Step 9. Configure Electrum Azart server

Electrum Azart reads a config file (/etc/electrum-azart.conf) when starting up. This
file includes the database setup, azartd RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-azart.conf
...
Electrum Azart server currently needs quite a few file handles to use leveldb. It also requires
file handles for each connection made to the server. It's good practice to increase the
open files limit to 64k.

The "configure" script will take care of this and ask you to create a user for running electrum-azart-server.
If you're using the user `azart` to run electrum and have added it as shown in this document, run
the following code to add the limits to your /etc/security/limits.conf:

     echo "azart hard nofile 65536" >> /etc/security/limits.conf
     echo "azart soft nofile 65536" >> /etc/security/limits.conf
...
Check if the limits are changed either by logging with the user configured to run Electrum Azart server as. Example:
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u azart -i ulimit -n


Two more things for you to consider:

1. To increase privacy of transactions going through your server
   you may want to close azartd for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting azartd (together with electrum-server) on a weekly basis to clear out unconfirmed
   transactions from the local the memory pool which did not propagate over the network.

### Step 11. (Finally!) Run Electrum Azart server

The magic moment has come: you can now start your Electrum Azart server as root (it will su to your unprivileged user):
...
You should see this in the log file:
    ...
    starting Electrum Azart server
...
If you want to stop Electrum Azart server, use the 'stop' command:
...
### Step 12. Test the Electrum Azart server
...
current server and connect to your new Electrum Azart server. You should see your
addresses and transactions history. You can see the number of blocks and
response time in the server selection window. You should send/receive some
azarts to confirm that everything is working properly.
...
If you're operating a public Electrum Azart server please subscribe
