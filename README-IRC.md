IRC is used by Electrum Azart server to find 'peers' - other Electrum Azart servers. The
current list can be seen by running:

  electrum-azart-server peers

The following config file options are used by the IRC part of Electrum Azart server:
...
the Electrum Azart server will run in private mode (default). In private
mode, `electrum-azart-server peers` will always return an empty list.

`host` is a fully-qualified domain name (FQDN) of your Electrum Azart
...
your Electrum Azart server instead of `host`. It is used as part of the name
...
composing the IRC nickname to identify your server on #electrum-azart .
